...
def version_file_data(version_type, file_path, file_list):
    datas = [] # datas로 이름 변경했음 주혜가.
    if version_type == "WORK":
        try:
            data.work_table.cellClicked.disconnect()
        except TypeError:
            print("이상하네")
    ...
    if version_type == "PUB":
        try:
            data.pub_table.cellClicked.disconnect()
        except TypeError:
            print("이상하네2")
            pass  # Ignore if there are no connections yet
        except RuntimeError:
    ...
    if version_type == "WORK" :
        if not file_path == "" :
            for file in file_list :
                datas.append((file[0], file[1], file[2], file[3]))
        else : 
            datas = [(f"/nas/eval/elements/null.png", "no work yet", "", "")]
    ...
    if version_type == "WORK":
        data.work_table.setRowCount(0)
        
        #work_table.cellClicked.connect(on_work_cell_clicked)
        for item in datas:
            file_table_item(data.work_table, *item)

    elif version_type == "PUB":
        data.pub_table.setRowCount(0)
        for item in datas:
            file_table_item(data.pub_table, *item)